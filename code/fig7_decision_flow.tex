...
\node[outb, right=of d3, xshift=6mm] (o2) {Between the two thresholds: at the tested sizes a partially pooled real-only estimator had the lower mean error here (by 0.2--0.7~pp); benchmark the calibrated panel against it \emph{and} direct estimation on held-out splits, and keep whichever wins};
...
\node[outb, below=of d4] (o4) {Calibrated synthetic panel, correction form selected inside the calibration set\\(6--9~pp at $n{\approx}80$--430; margin over a real-only shrinkage estimator $\le$2.3~pp, and $<$1~pp from $n{\approx}170$);\\always report the real-only comparison};
